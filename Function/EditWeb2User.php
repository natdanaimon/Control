...
$db->conn();
$sql = array(
    array("query" => "DELETE FROM web2user WHERE s_user = '$_POST[user_add]' ")
);
...
foreach ($_POST[my_multi_select2] as $var) {
    $strSQL = "insert into web2user (i_id , s_user) values ($var,'$_POST[user_add]') ";
    $sql2 = array(
        array("query" => "$strSQL")
    );
